...
ASM manages a project-local `.asm/` skill graph and syncs it into Cursor / Claude / Codex / GitHub Copilot.
One CLI replaces `npx skills`, `npx playbooks`, and manual skill management.

- **One CLI for all agents**: `asm search` + `asm add skill` — agents are instructed to never use other skill CLIs.
...
- **Federated Search**: Searches ASM index, Smithery, Playbooks, skills.sh, SkillsMP, and GitHub in parallel.
- **Reproducible Graphs**: `asm.toml` + `asm.lock` pin exact skill sources and hashes.
- **Zero-Touch Sync**: Keeps Cursor / Claude / Codex / Copilot configs in sync with your `.asm/` graph.

[Quick Start](#quick-start) · [Example workflows](#example-workflows) · [Usage](#usage) · [How Agents Use ASM](#how-agents-use-asm) · [How skill graphs work](#how-skill-graphs-work) · [CLI map](#cli-map)

## TL;DR

- **What ASM is**: A project-local skill manager that replaces `npx skills`, `npx playbooks`, and manual skill CLIs. It installs skills into `.asm/`, bundles them into *expertises*, and syncs a single `main_asm.md` index that your IDE agents read first.
- **Why you care**: New projects (or new machines) get the exact same agent "brain" in one command (`asm sync`). No more copy-pasting prompts and skills by hand, and no more agents installing skills with the wrong CLI.
- **Key contract**: Once ASM is initialized in a project, **all** skill search and install goes through `asm`. Agents are instructed to never use `npx skills`, `npx playbooks`, or other skill CLIs.
- **Fast path**:
...
IMPORTANT: Once ASM is set up, ALWAYS use `asm search` and `asm add skill` for finding and installing skills.
NEVER use `npx skills`, `npx playbooks`, or any other skill CLI — they bypass ASM and break the skill graph.

Do this in order:

1. Check whether `asm` is already installed (`asm --version`). If not, install it with:
...
4. Search and install only the most relevant skills.
   - Use `asm search "<query>"` to find skills.
   - Use `asm add skill <source>` to install (use the `source:` value from search output).
   - NEVER use `npx skills add` or `npx playbooks add`.
...
1. [playbooks] sql
   ...
   install: asm add skill pb:openclaw/skills/sql
...
The `install:` line gives you a ready-to-copy command. Always use `asm add skill`, never `npx skills add`.
...
When ASM is initialized in a project, every agent (Cursor, Claude Code, Codex, GitHub Copilot) gets a generated skill file that instructs it to:

1. **Use `asm search` to find skills** — not `npx skills find`, not `npx playbooks find`.
2. **Use `asm add skill <source>` to install** — not `npx skills add`, not manual cloning.
3. **Route tasks through expertises** — read `.asm/main_asm.md` for the skill graph.

This is enforced by the generated SKILL.md that ASM writes into each agent's config location. The generated file:

- Has trigger words ("find", "discover", "search", "install") in its description so agents match it before generic skill-finding tools.
- Shows a complete worked example of `asm search` → `asm add skill` so agents follow the concrete workflow.
- Explicitly prohibits `npx skills`, `npx playbooks`, and other CLIs.

If an agent ignores these instructions, it's usually because a competing global skill (like `find-skills`) was matched first. The fix is to ensure ASM's description better matches the user's intent, which ASM already handles.
